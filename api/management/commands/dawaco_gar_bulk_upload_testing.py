...
def handle_gar_delivery(
    ...
    write_to_csv(
        "gars_to_create.csv",
        [
            gar_df.iloc[0]["nitg_code"],
            uploadtask_sourcedocument_data.gmwBroId,
            uploadtask_sourcedocument_data.tubeNumber,
            uploadtask_sourcedocument_data.fieldResearch.samplingDateTime,
            uploadtask_sourcedocument_data.groundwaterMonitoringNets[0],
        ],
    )
...
def setup_gar_sourcedocs_data(
    ...
    if gmw_objects.count() == 0:
        ...
        write_to_csv("gmws_not_found.csv", [nitg_code])
        return None
    else:
