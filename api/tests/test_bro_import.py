...
def bulk_importer(importtask):
    return bulk_import.BulkImporter(
        importtask.uuid
    )
...
def test_create_bro_ids_import_url(bulk_importer):
    ...
    assert url == f"{settings.BRO_UITGIFTE_SERVICE_URL}/gm/gmn/v1/bro-ids?bronhouder=12345678"
...
def test_fetch_bro_ids_succes(mocker, bulk_importer):
    ...
    bro_ids = bulk_importer._fetch_bro_ids(f"{settings.BRO_UITGIFTE_SERVICE_URL}/gm/gmn/v1/bro-ids?bronhouder=12345678")
