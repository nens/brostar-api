...
class GMWDuplicatesHandler:
    ...
    def get_duplicates(self, properties: list[str] = ["well_code"]):
        ...
        for feature in self.features:
            feature["properties"].pop("delivered_location")
            bro_id = feature["properties"].get("bro_id")
            if not bro_id:
                continue
    ...
    def rank_duplicates(self):  # noqa: C901
        ...
        def amount_of_tubes_ranking(features: list[dict]):
            ...
            current_rank = 0
            tubes_sorted = sorted(
                tubes.items(), key=lambda x: -x[1]
            )  ## descending sorting for n_tubes
        ...
        def get_overall_ranking(features: list, duplicates: dict, rankings: dict):  # noqa: C901
            features_ranked = []
            ranks = {}
            scores = {}
            ...
            def convert_scores_to_ranks(
                scores: dict[str, dict[str]], features: list[dict]
            ) -> dict:
                # Preprocess: Map bro_id to registration_time
                times = {}
                for feature in features:
                    bro_id = feature.get("properties", {}).get("bro_id")
                    time_str = feature.get("properties", {}).get(
                    ...
                    if bro_id and time_str:
                        try:
                            ...
                            times[bro_id] = (
                                datetime.min
                            )  # fallback to oldest if parse fails
                ...
                for prop, bro_scores in scores.items():
                    ...
                    sorted_items = sorted(
                        scored_items, key=lambda x: (-x[1], -x[2].timestamp())
                    )
        ...
        def score_feature(ranking: dict, bro_id: str) -> float:  # noqa: C901
            ranks_weighted = []
            sum_weights = 0
        ...
        for (prop_name, prop), bro_ids in duplicates.items():
            ...
            ranking[prop] = {}
            ranking[prop][RANKING.NAMES.TUBE_RANKING] = amount_of_tubes_ranking(
                features
            )
            ranking[prop][RANKING.NAMES.QUALITY_RANKING] = quality_regime_ranking(
                features
            )
            ranking[prop][RANKING.NAMES.UKNONWN_RANKING] = unknown_values_ranking(
                features
            )
            ranking[prop][RANKING.NAMES.EMPTY_RANKING] = empty_values_ranking(features)
            ranking[prop][RANKING.NAMES.DATE_RANKING] = well_construction_date_ranking(
                features
            )

        duplicate_ids = [
            bro_id for bro_ids in duplicates.values() for bro_id in bro_ids
        ]
